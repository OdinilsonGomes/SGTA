...
$(() => {
  data_table = $("#dataTable").DataTable({
    ...
    order: [],
    ajax: {
      url: "servicos/cms.php?action=fetchAllTransferencia",
      type: "POST",
      data: {},
    },
    oLanguage: {
...
$('#transferir_modal, #transferir_modal_update').on('shown.bs.modal', function (e) {
  loadSelectTurma();
  
});
$('#transferir_modal, #transferir_modal_update, #remove_modal').on('hide.bs.modal', function (e) {
  clear_form();
})
...
function update(transferencia) {
  // Eliminando qualquer atributo selected 
  $('#turma_tranferir_update option').removeAttr('selected')
  // Atribuindo o valor ao select de acordo a tranferencia selecionada
  $('#turma_tranferir_update option[value='+transferencia.id_turma_destino+']').attr('selected','selected');
  $("#id_tranferirencia_update").val(transferencia.id);
  $("#data_transferir_update").val(transferencia.data);
  $("#motivo_transferir_update").val(transferencia.motivo);
  $("#transferir_modal_update").modal("show");
...
function updateAsync() {
  var id 		= $("#id_tranferirencia_update").val(),
    data 		= $("#data_transferir_update").val(),
    motivo 		= $("#motivo_transferir_update").val(),
    email 		= $("#email_aluno_update").val(),
    id_turma_destino 	= $("#turma_tranferir_update").val(),
	tips 		= $("#update_state");
  var formData  = new FormData();
  ...
  $.ajax({
    type: "POST",
    url: "servicos/cms.php?action=updateTransferencia",
    data: formData,
    contentType: false,
    processData: false,
    cache: false,
    success: function (data) {
      try {
        ...
        if (parseInt(r.result) != NaN && parseInt(r.result) == 1) {
          ...
          data_table.ajax.reload();
          fechar_modal("transferir_modal_update");
...
function removeAsync() {
  ...
  tips.html("<img src='img/loader.gif' />");
  $.post(
    "servicos/cms.php?action=removeTransferencia",
    {
      id: $("#id_to_remove").val(),
    },
    (data, status) => {
    ...
    (data, status) => {
      if (status == "success") {
        try {
          ...
          $("#turma_tranferir_update").html(html);
        } catch (error) {
          console.log(error);
        }
      } else {
